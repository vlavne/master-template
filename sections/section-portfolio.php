...
<section class="portfolio marg" id="<?php echo get_sub_field('unique_id'); ?>" data-aos="fade-up">
                ...
                <div class="portfolioBlockWrap">
                    <?php foreach ($item as $p): ?>
                        <a href="<?php echo $p['img']['url']; ?>" class="portfolioBlockItem"
                           data-fancybox="<?php echo get_sub_field('unique_id'); ?>"
                           data-caption="<strong><?php echo $p['title']; ?></strong><?php if ($p['info'] !== ''): ?><br><?php echo $p['info']; ?><?php endif; ?>">
    ...
    ?>
    <?php if ($btnLink): ?>
        <a href="<?php echo $btnLink['url']; ?>" class="btn" data-aos="fade-up"
           <?php if ($btnLink['target']): ?>target="<?php echo $btnLink['target']; ?>"<?php endif; ?>><?php echo $btnLink['title']; ?></a>
    <?php elseif ($btn): ?>
        <div class="btn" data-aos="fade-up"
             onclick="$(this).hide();$(this).prev('.portfolioBlock').addClass('showAll');"><?php echo $btn; ?></div>
    <?php endif; ?>
        ...
        <?php foreach (get_sub_field('portfolio') as $p): ?>
            <div class="portfolioBlockMobItem">
                <a href="<?php echo $p['img']['url']; ?>"
                   data-fancybox="<?php echo get_sub_field('unique_id'); ?>2"
                   data-caption="<strong><?php echo $p['title']; ?></strong><?php if ($p['info'] !== ''): ?><br><?php echo $p['info']; ?><?php endif; ?>">
                    <?php echo wp_get_attachment_image($p['img']['ID'], 'large'); ?>
                </a>
            </div>
